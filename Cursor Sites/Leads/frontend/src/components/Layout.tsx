  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white border-b sticky top-[73px] shadow-md" style={{ zIndex: 1000, position: 'relative', pointerEvents: 'auto' }}>
              ...
              {menuItems.map((item) => {
                ...
                if (hasSubmenu) {
                  return (
                    ...
                    >
                      <button
                        className={`inline-flex items-center px-4 py-2 rounded-md text-sm font-semibold transition-colors whitespace-nowrap ${
                          isActive(item.path)
                      ...
                      {dropdownOpen === item.path && (
                        <div 
                          className="absolute left-0 mt-2 w-72 bg-white rounded-lg shadow-2xl border border-gray-200 py-2"
                          style={{ 
        ...
        {/* Mobile menu */}
        {mobileMenuOpen && (
          <div className="lg:hidden border-t border-gray-200 bg-white" style={{ zIndex: 1001 }}>
