...
class Recovered_Report_Table extends \WP_List_Table {
	...
	public function recovered_table() {
        ...
        <div class="wacv-abanoned-cart-page">
            <ul class="subsubsub">
                <li><a href="<?php echo esc_url( admin_url( 'admin.php?page=wacv_recovered&recovered=all_customer' ) ) ?>"
                       class="<?php echo esc_html( $class_all_customer ) ?>"><?php esc_html_e( 'All', 'woo-abandoned-cart-recovery' ) ?></a>
                    |
                </li>
                <li><a href="<?php echo esc_url( admin_url( 'admin.php?page=wacv_recovered&recovered=member' ) ) ?>"
                       class="<?php echo esc_html( $class_member ) ?>"><?php esc_html_e( 'Member', 'woo-abandoned-cart-recovery' ) ?></a>
                    |
                </li>
                <li><a href="<?php echo esc_url( admin_url( 'admin.php?page=wacv_recovered&recovered=guest' ) ) ?>"
	...
	public function get_columns() {
		$columns = array(
			...
			'id'           => esc_html__( 'ID', 'woo-abandoned-cart-recovery' ),
			'customer'     => esc_html__( 'Customer', 'woo-abandoned-cart-recovery' ),
			'email'        => esc_html__( 'Email Address', 'woo-abandoned-cart-recovery' ),
			'order_detail' => esc_html__( 'Recovered Cart Detail', 'woo-abandoned-cart-recovery' ),
			'coupon'       => esc_html__( 'Coupon used', 'woo-abandoned-cart-recovery' ),
			'date'         => esc_html__( 'Recovered Date', 'woo-abandoned-cart-recovery' ),
			'more_info'    => esc_html__( 'More Info', 'woo-abandoned-cart-recovery' )
		);
...
//		check($data);die;
		...
		foreach ( $results as $result ) {
			...
			if ( $result->user_type == 'guest' ) {
				...
				if ( count( $results_guest ) > 0 ) {
					...
					$name       = ! empty( trim( $name ) ) ? $name : esc_html__( 'Guest', 'woo-abandoned-cart-recovery' );
				} else {
					$user_email = '';
					$name       = esc_html__( 'Visitor', 'woo-abandoned-cart-recovery' );
				}
			}
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'coupon':
				return $item->coupon;
			case 'order_detail':
				$out = "<table class='wacv-recovered-detail-tb'><tr><td>" . esc_html__( 'Total: ', 'woo-abandoned-cart-recovery' ) . "</td><td>" . $item->total . "</td></tr>";
				$out .= "<tr><td>" . esc_html__( 'Tax: ', 'woo-abandoned-cart-recovery' ) . "</td><td>" . $item->tax . "</td></tr>";
				$out .= "</table>";
	...
	public function column_cb( $item ) {
		return sprintf( '<input type="checkbox" name="wacv_abandoned_id[]" value="%s" />', esc_attr( $item->id ) );
	}
