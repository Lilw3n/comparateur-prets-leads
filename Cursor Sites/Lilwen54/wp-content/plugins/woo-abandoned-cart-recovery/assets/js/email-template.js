    ...
    const getViewBlock = {
        ...
        buttonField: function (field) {
            let html = '<div class="wacv-block" data-type="button" data-field="' + field + '">' + this.handle(field) +
                ...
                '               <tr >' +
                '                   <td style="color:inherit; background-color: inherit; font-size: inherit;text-align: inherit;">' +
                `               <div type="button" class="wacv-button-text wacv-background-color-output" style="width:50%; border: none; padding: 5px 0; margin: auto; color: inherit; background-color: #212121;" >` +
                `              <p style="text-align: center;"><span style="font-size: 14pt;"><strong><a href="{wacv_checkout_btn}">Checkout Now</a></strong></span></p></div>` + // <a href="{wacv_checkout_btn}" style=" text-decoration: none;">
                '                   </td>' +
                '               </tr>' +
                '            </table>' +
                '            </div>' +
        ...
        init: function () {
            ...
            el.each(function () {
                ...
                let outputStyle = $this.find('.wacv-output').attr('style'),
                    ...
                    output = '';
                if (data_type === 'cart') {
                    content = content.replace('<tr data-cart="{wacv_cart_detail_start}"></tr>', '{wacv_cart_detail_start}');
                    content = content.replace('<tr data-cart="{wacv_cart_detail_end}"></tr>', '{wacv_cart_detail_end}');
                    content = content.replace(/http(s?):\/\/.*product.png/i, '{wacv_image_product}');
                    content = content.replace(/http(s?):\/\/.*product-4.jpg/i, '{wacv_image_product}');
                }
    ...
    const controlPanel = {
        ...
        editor: function (target) {
            tinymce.init({
                ...
                toolbar: ['bold italic underline | alignleft aligncenter alignright | link ', 'fontsizeselect forecolor backcolor ', 'shortcode fontselect'],
                setup: function (editor) {
                    editor.addButton('shortcode', {
                        ...
                        values: [
                            ...
                            {text: '{coupon_expire}', value: '{coupon_expire}'},
        ...
        eventCtrl: function () {
            ...
            $('.wacv-button-width').on('change', function () {
                ...
                $('.li_' + field_focus).find('.wacv-output .wacv-button-text').css('width', val + '%');
            ...
            $('.wacv-element-align').on('change', function () {
                let val = $(this).val();
                let block_type = $('.li_' + field_focus).find('.wacv-output').closest('.wacv-block').data('type');

                $('.li_' + field_focus).find('.wacv-output').css('text-align', val);
                if ( block_type === 'button' ) {
                    let button_wrap = $('.li_' + field_focus).find('.wacv-output div.wacv-button-text');
                    switch (val) {
                        case 'left':
                            $(button_wrap).css({
                                'float': 'left',
