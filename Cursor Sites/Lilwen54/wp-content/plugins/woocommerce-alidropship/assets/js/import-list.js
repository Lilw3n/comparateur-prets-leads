jQuery(document).ready(function ($) {
    'use strict';
    let queue = [], body = $('body'),
        findProductModal = $('#ald-find-product-modal').modal({autofocus: false,}),
        _vi_wad_ajax_nonce = vi_wad_import_list_params._vi_wad_ajax_nonce,
        is_importing = false;
    ...
    function generate_cat_name(name, parent_category_id, cats) {
        if (!name || !parent_category_id) {
            return name;
        }
        for (let cat of cats) {
            let {category_id, category_name, parent_id} = cat;
            if (category_id == parent_category_id) {
                name = `${category_name} > ${name}`;
                if (parent_id) {
                    name = generate_cat_name(name, parent_id, cats);
                    break;
                }
            }
        }
    ...
    function change_author_dokan() {
        $('body').on('click', '.vi-wad-edit-icon', function (e) {
            e.stopPropagation();
            let t = $(this),
                t_container = t.closest('.vi-wad-accordion-product-vendor'),
                t_parent = t.parent(),
                t_choose_seller_wrap = t_container.find('.vi-wad-list-all-dokan-seller');
            t_parent.hide();
            t_choose_seller_wrap.show();
            return false;
        });
        $('body').on('click', '.vi-wad-cancel-author', function (e) {
            e.stopPropagation();
            let t = $(this),
                t_container = t.closest('.vi-wad-accordion-product-vendor'),
                t_parent = t.closest('.vi-wad-list-all-dokan-seller'),
                t_seller_wrap = t_container.find('.vi-wad-seller');
            t_parent.hide();
            t_seller_wrap.show();
            return false;
        });
        $('body').on('click', '.vi-wad-save-author', function (e) {
            e.stopPropagation();
            let t = $(this),
                t_icon = t.find('.icon'),
                t_accordion = t.closest('.vi-wad-accordion'),
                t_container = t_accordion.find('.vi-wad-accordion-product-vendor'),
                t_parent = t_accordion.find('.vi-wad-list-all-dokan-seller'),
                t_seller_wrap = t_container.find('.vi-wad-seller'),
                choose_author = t_parent.find('.vi-wad-dokan-sellers'),
                current_choose_author = choose_author.val(),
                product_id = t_accordion.data('product_id');
            t_icon.removeClass('check').addClass('sync loading');
            $.ajax({
                url: vi_wad_import_list_params.url,
                type: 'POST',
                dataType: 'JSON',
                data: {
                    action: 'vi_wad_dokan_admin_edit_author',
                    _vi_wad_ajax_nonce: _vi_wad_ajax_nonce,
                    product_id: product_id,
                    author_id: current_choose_author,
                },
                success: function (response) {
                    if (response.status === 'success') {
                        t_seller_wrap.find('span:not(.vi-wad-edit-icon)').html(response.vendor_name)
                ...
                complete: function () {
                    t_icon.removeClass('sync loading').addClass('check');
                    t_parent.hide();
                    t_seller_wrap.show();
                }
            });
    ...
    function init() {
        ...
        $('.vi-wad-modal-popup-set-shipping-class-select,.vi-wad-import-data-shipping-class,.vi-wad-show_product_video_tab').dropdown({placeholder: '', fullTextSearch: true});
        $('.vi-wad-import-data-tags,.vi-wad-modal-popup-set-tags-select').dropdown({
            allowAdditions: vi_wad_import_list_params.tags_allow_addition,
            fullTextSearch: true
        });
        $('.ald-search-product-cate').dropdown({
            fullTextSearch: true
        });
        $('.ald-ship-to-country').dropdown({
            fullTextSearch: true,
            onChange: function (val) {
                let $prefix = val.toString().toLowerCase() + '_',
                    $sort_options = $('.ald-search-product-sort select'),
                    $cat_options = $('.ald-search-product-cate select'),
                    $cat_option = $(`.ald-search-product-cate-options`);
                let sort_options = $sort_options.data($prefix + 'options') || $sort_options.data('options');
                if ($(`.ald-search-product-cate- ${$prefix}options`).length) {
                    $cat_option = $(`.ald-search-product-cate- ${$prefix}options`);
                }
                let cat_options = $cat_option.data('options');
                if (Object.keys(sort_options).length) {
                    let $sort_options_value = '';
                    $sort_options.html('');
                    $.each(sort_options, function (k, v) {
                        if (!$sort_options_value) {
                            $sort_options_value = k;
                        }
                        $sort_options.append(`<option value="${k}">${v}</option>`);
                    });
                    if ($sort_options_value) {
                        $('.ald-search-product-sort').dropdown('set selected', $sort_options_value);
                    }
                }
                if (Object.keys(cat_options).length) {
                    let $sort_options_value = ' ';
                    $cat_options.html($cat_options.find('option').eq(0).clone());
                    for (let cat of cat_options) {
                        let {category_id, category_name, parent_category_id} = cat;
                        if (parent_category_id) {
                            category_name = generate_cat_name(category_name, parent_category_id, cat_options);
                        }
                        $cat_options.append(`<option value="${category_id}">${category_name}</option>`);
                    }
                    $('.ald-search-product-cate').dropdown('set selected', $sort_options_value);
                }
            }
        });
        ...
        $('.ald-product-categories').each(function () {
            let cats = $(this).data('categories');
            $(this).find('.vi-wad-import-data-categories').dropdown('set selected', cats).trigger('change');
        });

        if (vi_wad_import_list_params.product_categories) {
            $('.vi-wad-import-data-categories').dropdown('set selected', vi_wad_import_list_params.product_categories).trigger('change');
        }

        $('.vi-wad-override-woo-id').select2({
            closeOnSelect: true,
            allowClear: true,
            placeholder: "Please enter product title to search",
            ajax: {
                url: `admin-ajax.php?action=wad_search_product&_vi_wad_ajax_nonce=${_vi_wad_ajax_nonce}&exclude_ali_products=1`,
                dataType: 'json',
                type: "GET",
                quietMillis: 50,
                delay: 250,
        ...
        $('.vi-wad-override-product-id').select2({
            closeOnSelect: true,
            allowClear: true,
            placeholder: "Please enter product title to search",
            ajax: {
    ...
    $(document).on('keydown', function (e) {
        ...
        if (!$container.hasClass('vi-wad-hidden')) {
            if (e.keyCode == 13) {
                if ($container.hasClass('vi-wad-modal-popup-container-set-categories')) {
                    $('.vi-wad-set-categories-button-add').click();
                } else if ($container.hasClass('vi-wad-modal-popup-container-set-shipping-class')) {
                    $('.vi-wad-set-shipping-class-button-set').click();
                } else if ($container.hasClass('vi-wad-modal-popup-container-set-price')) {
    ...
    let $import_list_count = $('#toplevel_page_woocommerce-alidropship').find('.current').find('.vi-wad-import-list-count');
    ...
    body.on('click', '.vi-wad-button-import', function (e) {
        ...
        let form_data = $form.find('.vi-ui.tab').not('.vi-wad-variations-tab').find('input,select').serializeArray();
        ...
        if ($form.find('.vi-wad-variation-enable').length > 0) {
            ...
            $form.find('.vi-wad-variation-enable').map(function () {
                ...
                if ($(this).prop('checked') && !$row.hasClass('vi-wad-variation-filter-inactive')) {
                    each_selected.push($row.find('.vi-wad-import-data-variation-sku-attr').val());
        ...
        if (selected[product_id].length === 0) {
            villatheme_admin_show_message(vi_wad_import_list_params.i18n_empty_variation_error, 'error', '', false, 5000);
            return;
        }
        ...
        if (empty_price_error) {
            villatheme_admin_show_message(vi_wad_import_list_params.i18n_empty_price_error, 'error', '', false, 5000);
            return;
        } else if (sale_price_error) {
            villatheme_admin_show_message(vi_wad_import_list_params.i18n_sale_price_error, 'error', '', false, 5000);
            return;
        }
        ...
        if (!is_importing) {
            ...
            if (vi_wad_import_list_params.php_version && (parseFloat(vi_wad_import_list_params.php_version) >= 8.3)) {
                new_formdata = btoa(JSON.stringify(form_data));
            } else {
                new_formdata = form_data;
            }
    ...
    body.on('click', '.vi-wad-button-import-all', function () {
        ...
        $('.vi-wad-button-import').not('.loading').not('.vi-wad-hidden').map(function () {
        ...
        if (queue.length > 0) {
            ...
            villatheme_admin_show_message(vi_wad_import_list_params.i18n_not_found_error, 'error', '', false, 5000);
        }
    });
    ...
    body.on('click', '.vi-wad-override-product-options-button-override', function () {
        ...
        let $product_container = $('#vi-wad-product-item-id-' + product_id);
        let $form = $product_container.find('.vi-wad-product-container');
        let $overlay = $product_container.find('.vi-wad-product-overlay');
        // let data = $form.serializeArray();
        ...
        $.ajax({
            ...
            data: {
                ...
                override_video: $('.vi-wad-override-product-options-override-video').prop('checked') ? 1 : 0,
                override_images: $('.vi-wad-override-product-options-override-images').prop('checked') ? 1 : 0,
                override_specifications: $('.vi-wad-override-product-options-override-specifications').prop('checked') ? 1 : 0,
                ...
                migration_link_only: vi_wad_import_list_params.migration_link_only,
            },
            success: function (response) {
                if (check_orders) {
                    if (response.status === 'success') {
                        ...
                        villatheme_admin_show_message(response.message, 'error', '', false, 5000);
                    }
                } else {
                    if (response.status === 'checked') {
                        ...
                        $replace_order.find('.vi-wad-override-with-attributes>select').trigger('change');
                        check_orders = 1;
    ...
    $(document).on('change', '.vi-wad-override-with-attributes>select', function (e) {
        ...
        if ($container.hasClass('vi-wad-override-product-options-container-reimport') || ($container.hasClass('vi-wad-override-product-options-container-map-existing') && !vi_wad_import_list_params.migration_link_only) || (!$container.hasClass('vi-wad-override-product-options-container-map-existing') && $('.vi-wad-override-product-options-override-keep-product').prop('checked'))) {
            let $current = $(this);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status === 'success') {
                        ...
                        $tab.closest('.vi-wad-product-container').find('.vi-wad-selected-variation-count').html(parseInt(response.variations_count))
                    } else {
                    ...
                    let $rows = $tbody.find('.vi-wad-product-variation-row');
                    $rows.map(function () {
                        let $row = $(this);
                    ...
                    let $row_no = 1;
                    $tbody.find('.vi-wad-product-variation-row-number').map(function () {
                        $(this).html($row_no++)
                    });
    ...
    function vi_wad_override_product_show($button_import) {
        ...
        if ($override_options.length > 0) {
            if ($button_import.hasClass('vi-wad-button-map-existing') && vi_wad_import_list_params.migration_link_only) {
                $override_options.addClass('vi-wad-hidden');
                $('.vi-wad-override-product-options-button-override').click();
            } else {
     ...
     * Split product
     */
    $(document).on('click', '.vi-wad-button-split', function () {
        let $button = $(this);
        let $container = $button.closest('.vi-wad-accordion');
        let $overlay = $container.find('.vi-wad-product-overlay');
        let $variations_tab = $button.closest('.vi-wad-variations-tab');
        let $button_container = $button.closest('.vi-wad-button-split-container');
        let product_id = $button.data('product_id');
        ...
        if (!split_attribute_id) {
            $variations_tab.find('.vi-wad-product-variation-row').not('.vi-wad-variation-filter-inactive').map(function () {
                if ($(this).find('.vi-wad-variation-enable').prop('checked')) {
                    split_variations_ids.push($(this).find('.vi-wad-import-data-variation-sku-attr').val());
                }
            });
            if (split_variations_ids.length === 0) {
                villatheme_admin_show_message(vi_wad_import_list_params.i18n_split_product_no_variations, 'error', '', false, 5000);
                return;
            } else if (split_variations_ids.length === $variations_tab.find('.vi-wad-product-variation-row').length) {
                villatheme_admin_show_message(vi_wad_import_list_params.i18n_split_product_too_many_variations, 'error', '', false, 5000);
                return;
            } else if (!confirm(`${vi_wad_import_list_params.i18n_split_product_confirm} ${vi_wad_import_list_params.i18n_split_product_message}`)) {
                return;
            }
        } else {
            if (!confirm($button.data('split_product_message') + ` ${vi_wad_import_list_params.i18n_split_product_message}`)) {
                return;
            }
        }
        $button.addClass('loading');
    ...
    /*Edit attributes*/
    $(document).on('click', '.vi-wad-specification-attribute-remove', function () {
    ...
    $(document).on('click', '.vi-wad-attributes-button-save', function () {
        ...
        if (!$name.val()) {
            villatheme_admin_show_message(vi_wad_import_list_params.i18n_empty_attribute_name, 'error', '', false, 5000);
            $name.focus();
            return;
        }
        ...
        if (attribute_values.length !== $attribute_values.length) {
            villatheme_admin_show_message(vi_wad_import_list_params.i18n_invalid_attribute_values, 'error', '', false, 5000);
            return;
        }
        if (change > 0) {
            $overlay.removeClass('vi-wad-hidden');
                    ...
                    if (response.status === 'success') {
                        ...
                        if (response.change_value === true) {
                            ...
                            $row.find('.vi-wad-attributes-attribute-value').map(function () {
                                ...
                                $attribute_value.data('attribute_value', $attribute_value.val()).trigger('change');
                            });
                        }
    ...
    body.on('click', '.vi-wad-description-tab-menu', function () {
        $(`.vi-wad-description-tab[data-tab="${$(this).data('tab')}"]`).find('.switch-tmce').trigger('click');
    });
    ...
    body.on('click', '.vi-wad-modal-popup-select-attribute .vi-wad-attributes-attribute-value', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    if ($container.find('.vi-wad-attributes-attribute-row:not(.vi-wad-specification-attribute-row)').length > 1) {
                        $row.remove();
    ...
    body.on('change', 'select[name="vi_wad_bulk_actions"]', function () {
        ...
        if ($checkbox.length > 0 && action !== '') {
            switch (action) {
                ...
                case 'set_categories':
                    ...
                    let $container = $('.vi-wad-modal-popup-container');
                    $container.attr('class', `vi-wad-modal-popup-container vi-wad-modal-popup-container-set-${taxonomy.replace(/_/g, '-')}`);
                    vi_wad_set_price_show();
                    break;
                case 'import':
    ...
    body.on('click', '.vi-wad-set-shipping-class-button-set', function () {
        let $checkbox = $('.vi-wad-accordion-bulk-item-check'),
            $new_shipping_class = $('select[name="vi_wad_bulk_set_shipping_class"]'),
            new_shipping_class = $new_shipping_class.val();
        $checkbox.map(function () {
            let $button = $(this);
            if ($button.prop('checked')) {
                let $container = $button.closest('.vi-wad-accordion'),
                    $shipping_class = $container.find('.vi-wad-import-data-shipping-class');
    ...
    body.on('click', '.vi-wad-set-shipping-class-button-cancel', function () {
        vi_wad_set_price_hide();
    });
    ...
    let getAccessTokenModal = $('#ald-get-access-token-modal').modal({allowMultiple: true});
    let getAccessTokenModalContent = getAccessTokenModal.find('.ald-generate-access-token-wrapper');
    let cacheButtonClicking;
    ...
    var importing = false, importQueue = [];


    body.on('click', '.ald-open-find-product-modal', function () {
        if (!$('.ald-ship-to-country').dropdown('get value')) {
            $('.ald-ship-to-country').dropdown('set selected', $('.ald-ship-to-country select').data('shipto'));
        }
    ...
    body.on('submit', '.ald-search-product-form', function (e, data) {
        ...
        let sendData = `action=ald_search_product&_vi_wad_ajax_nonce=${_vi_wad_ajax_nonce}&` + formData;
    ...
    body.on('click', '.ald-add-to-import-list', function (e) {
        ...
        $thisBtn.addClass('ald-add-to-import-list-importing');
        $thisBtn.find('i.icon').addClass('spinner loading');
        importQueue.push(pid);
        if (importing) {
            return;
        }
        importing = true;
    ...
    function aldAddToImportList() {
        let pid, btn, data = {
            action: 'vi_wad_add_to_import_list',
            _vi_wad_ajax_nonce: _vi_wad_ajax_nonce
        };
        if (importQueue.length) {
            pid = importQueue.shift();
        } else {
            importing = false;
            data['reloadHtml'] = 1;
        }
        if (pid) {
            btn = $(`.ald-add-to-import-list[data-product_id=${pid}]`);
            if (btn.data('use_api')) {
                data['product_id'] = pid;
                data['country'] = $('.ald-ship-to-country').dropdown('get value');
            } else {
                window.open(btn.attr('href'), `Import${pid}`, "width=380,height=600");
                window.focus();
                return;
            }
        }
        $.ajax({
            ...
            success(response) {
                ...
                if (response?.status === 'success') {
                    if (response?.import_list) {
                        tinymce.remove();
                        $('.wrap.woocommerce-alidropship-import-list').replaceWith(response.import_list);
            ...
            complete() {
                if (importQueue.length || !data?.reloadHtml) {
                    aldAddToImportList();
                }
                if (btn) {
                    btn.find('.loading').removeClass('spinner loading');
        ...
        if (event?.data?.action) {
            ...
            switch (action) {
                case 'ald_reload_list':
                    ...
                    villatheme_admin_show_message('Successfully', 'success', `${btn.data('product_title')} is added to import list`, false, 5000);
                    break;
                case 'ald_notice_error':
                    villatheme_admin_show_message(message, 'error', '', false, 5000);
                    break;
            }
        }
    });
